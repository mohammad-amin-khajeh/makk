+++
title = 'French 2 cheatsheet'
description = "For those of us who don't know how to answer 'Qu'est-ce que vous faites dans la vie'"
date = "2024-09-25"
[extra]
toc = true
[taxonomies]
tags = ['french', 'university']
categories = ['cheatsheet']
+++

# verbs

## appeler or s'appeler?

- ### appeler

  appeler directly translates as 'to call' and is used when one person calls another person or thing.
  It can also be used in a variety of contexts,
  such as to summon someone or something, to make a phone call, or to give a name.

  | French Sentence | English translation |
  | -------------- | --------------- |
  |  Je vais **appeler** Paul au téléphone.  | {{ spoiler(text="I'm going to call Paul on the phone.") }} |
  |  Peux-tu **appeler** un taxi?  | {{ spoiler(text="Can you call a taxi?") }} |
  | Ils vont **appeler** leur fille Élise. | {{ spoiler(text="They are going to name their daughter Elise.") }} |

- ### S'appeler

  S'appeler is a reflexive form of appeler and means 'to be called' or 'to call oneself'.
  It is used when the subject is also the object, i.e. when referring to oneself, as in saying one's name.

  | French Sentence | English translation |
  | -------------- | --------------- |
  | Comment **t'appelles**-tu ? | {{ spoiler(text="What is your name?") }} |
  | Elle **s'appelle** Marie. | {{ spoiler(text="Her name is Marie") }} |
  |Nous nous **appelons** Jean et Luc. | {{ spoiler(text="Our names are Jean and Luc") }} |

- ### conjugation table

  | Singular               | Plural                |
  |-----------------------|----------------------|
  | Je m'appelle          | Nous nous appelons    |
  | Tu t'appelles         | Vous vous appelez     |
  | Il/Elle s'appelle     | Ils/Elles s'appellent |

  In summary, while both appeler and s'appeler refer to the concept of calling,
  appeler is used for calling others or naming things,
  whereas s'appeler refers to what someone is called or calling oneself by name.

## habiter

- ### conjugation table

  | Singular               | Plural                |
  |-----------------------|----------------------|
  | J'habite              | Nous habitons        |
  | Tu habites            | Vous habitez         |
  | Il/Elle habite        | Ils/Elles habitent   |

- ### Example sentences

  | French Sentence | English Translation |
  |---|---|
  | **J'habite** à Paris. | {{spoiler(text="I live in Paris.")}} |
  | **Nous habitons** dans une maison. | {{spoiler(text="We live in a house.")}} |
  | **Ils habitent** au Canada. | {{spoiler(text="They live in Canada.")}} |
  | **Où habitez-vous** ? | {{spoiler(text="Where do you live?")}} |
  | **Elle habite** avec ses parents. | {{spoiler(text="She lives with her parents.")}} |
  | **Tu habites** près de l'école. | {{spoiler(text="You live near the school.")}} |

- ### Country names along with the correct preposition

  | French Country Name | English Translation |
  |---|---|
  | **au** Canada | Canada |
  | **aux** États-Unis | the United States |
  | **en** France | France |
  | **en** Italie | Italy |
  | **en** Iran | Iran |
  | **en** Espagne | Spain |
  | **en** Allemagne | Germany |
  | **en** Chine | Chine |
  | **en** Angleterre | England |
  | **en** Suisse | Switzerland |
  | **au** Japon | Japan |
  | **en** Belgique | Belgium |
  | **aux** Pays-Bas | the Netherlands |

  So for instance we can say:

  | French Sentence | English Translation |
  |---|---|
  | **J'habite** à Paris, en France. | {{spoiler(text="I live in Paris, France.")}} |

## épeler

This verb means to spell and it is mostly used to ask someone to spell their name as in:

| French Sentence | English translation |
| -------------- | --------------- |
| Comment **épelez**-vous votre nom? | {{ spoiler(text="How do you spell your name?") }} |

- ### Conjugation table

  | Singular          | Plural            |
  |-------------------|-------------------|
  | j'épelle          | nous épelons      |
  | tu épelles        | vous épelez       |
  | il/elle épelle | ils/elles épellent|

## être

While we're at it let's also have a refresher and see how être is conjugated:

| Singular          | Plural            |
|-------------------|-------------------|
| je suis           | nous sommes       |
| tu es             | vous êtes         |
| il/elle est    | ils/elles sont    |

## Aller

**Aller** is a very common French verb meaning "to go".
It's one of the irregular verbs in French, so its conjugation is different from regular verbs.

- ### Conjugation Table

  | Singular          | Plural            |
  |-------------------|-------------------|
  | je vais          | nous allons      |
  | tu vas          | vous allez        |
  | il/elle va   | ils/elles vont |

- ### Example sentences

  | French Sentence | English Translation |
  |---|---|
  | **Je vais** au cinéma. | {{spoiler(text="I am going to the cinema.")}} |
  | **Tu vas** à la plage. | {{spoiler(text="You are going to the beach.")}} |
  | **Il va** au restaurant. | {{spoiler(text="He is going to the restaurant.")}} |
  | **Nous allons** à la fête. | {{spoiler(text="We are going to the party.")}} |
  | **Vous allez** au parc. | {{spoiler(text="You are going to the park.")}} |
  | **Ils vont** à l'école. | {{spoiler(text="They are going to school.")}} |
  | **Tu vas** bien? | {{spoiler(text="How are you?/Are you okay?")}} |

{{admonition(type='note'
text='The verb "aller" is also used to express future actions in French.
For example, "Je **vais** manger" means "I am going to eat."')}}

## Avoir

### Conjugation of Avoir

| Singular | Plural |
|---|---|
| j'ai | nous avons |
| tu as | vous avez |
| il/elle a | ils/elles ont |

### Sentence Examples

#### **Possession:**

| French | English |
|---|---|
| J'ai un chien. | {{spoiler(text="I have a dog.")}} |
| Elle a une voiture. | {{spoiler(text="She has a car.")}} |
| Nous avons une maison. | {{spoiler(text="We have a house.")}} |

#### **Existence:**

| French | English |
|---|---|
| Il y a un livre sur la table. | {{spoiler(text="There is a book on the table.")}} |
| Il n'y a pas de café. | {{spoiler(text="There is no coffee.")}} |

#### **Time:**

| French | English |
|---|---|
| J'ai faim. | {{spoiler(text="I am hungry.")}} |
| Il a soif. | {{spoiler(text="He is thirsty.")}} |
| Nous avons froid. | {{spoiler(text="We are cold.")}} |

#### **Age:**

| French | English |
|---|---|
| J'ai 25 ans. | {{spoiler(text="I am 25 years old.")}} |
| Tu as quel âge? | {{spoiler(text="How old are you?")}} |
| Ils ont 30 ans. | {{spoiler(text="They are 30 years old.")}} |

## naître

*Naître* means "to be born" in French. It's a regular verb, meaning its conjugation follows a predictable pattern.

### Conjugation Table(Past Indicative)

| Subject Pronoun | Conjugation |
|---|---|
| je | suis né(e) |
| tu | es né(e) |
| il/elle/on | est né(e) |
| nous | sommes né(e)s |
| vous | êtes né(e)s |
| ils/elles | sont né(e)s |

### Example sentences

| French | English |
|---|---|
|Ils sont nés à Paris| {{spoiler(text="They were born in paris")}} |
|Il est né au Japon| {{spoiler(text="He was born in Japan")}} |
|Nous sommes nés en Iran| {{spoiler(text="We were born in Iran")}} |
|Ils sont nés à New York| {{spoiler(text="They were born in New York")}} |

{{admonition(type='warning'
text='**Note:** The past participle of naître is né(e).
The masculine form is né, and the feminine form is née.')}}

# grammar

## quel and quelle

**Quel** and **quelle** are French interrogative adjectives used to ask questions about nouns.
Like "which" or "what" in English.

- **Quel:** Used before masculine singular nouns.
- **Quelle:** Used before feminine singular nouns.

- ### Sentence examples

  | French Sentence | English Translation |
  |---|---|
  | **Quelle** est votre profession? | {{spoiler(text="What do you do for a living?")}} |
  | **Quel** est ton/votre nom ? | {{spoiler(text="What's your name?")}} |
  | **Quelle** est votre adresse? | {{spoiler(text="What's your address?")}} |
  | **Quel** est votre numéro de téléphone?| {{spoiler(text="What's your telephone number?")}} |

## French Possessive adjectives

**Possessive adjectives** in French indicate ownership or possession. They agree in gender and number with the possessed item.

### List of French Possessive Articles

| English | Masculine Singular | Feminine Singular | Masculine Plural | Feminine Plural |
|---|---|---|---|---|
| My | mon | ma | mes | mes |
| Your (singular) | ton | ta | tes | tes |
| His/Her/Its | son | sa | ses | ses |
| Our | notre | notre | nos | nos |
| Your (plural) | votre | votre | vos | vos |
| Their | leur | leur | leurs | leurs |

### Usage examples

| French | English |
|---|---|
| Mon livre | {{spoiler(text='My book')}} |
| Ta voiture | {{spoiler(text='Your car')}} |
| Ses amis | {{spoiler(text='His/Her/Its friends')}} |
| Notre maison | {{spoiler(text='Our house')}} |
| Vos enfants | {{spoiler(text='Your children')}} |
| Leurs parents | {{spoiler(text='Their parents')}} |

{{admonition(type='warning'
text='Feminine nouns starting with h or a vowel typically take the
male article.\
So we have "mon ami" meaning my(male) friend,\
but also "mon amie", my(female) friend.')}}

## Understanding *c'est* and the Indefinite Article in French

**c'est** is a contraction of the verb *être* (to be) and the pronoun *ce*. It is used to say "it is" or "this is".

The indefinite article in French is used to refer to nouns that are not specific. It has two forms:

- **un** (masculine): a, one
- **une** (feminine): a, one

### Example Sentences

| French Sentence | English Translation |
|---|---|
| C'est un livre. | {{spoiler(text="It's a book.")}} |
| C'est une maison. | {{spoiler(text="It's a house.")}} |
| C'est un chat gris. | {{spoiler(text="It's a gray cat.")}} |
| C'est une fille intelligente. | {{spoiler(text="She's a smart girl.")}} |
| **Ce sont des amis.** | {{spoiler(text="They are friends.")}} |
| **Ce sont des étudiants.** | {{spoiler(text="They are students.")}} |

{{admonition(type='tip'
text="When referring to plural nouns, we use **ce sont** instead of **c'est**.")}}

## Understanding "il y a"

**"Il y a"** is a fundamental French expression that translates to "there is" or "there are." It's used to indicate the existence of something or someone.

### How to use "il y a"

- **To express quantity:**
  - *Il y a beaucoup de livres.* (There are many books.)
  - *Il y a peu de personnes.* (There are few people.)

- **To indicate a location:**
  - *Il y a un parc.* (There is a park.)
  - *Il y a des magasins.* (There are stores.)

- **To talk about time:**
  - *Il y a deux heures, j'étais à la maison.* (Two hours ago, I was at home.)
  - *Il y a longtemps que je ne l'ai pas vu.* (It's been a long time since I saw him.)

### Example sentences with various contexts

| French Sentence | English Translation |
|---|---|
| Il y a un chat. | {{spoiler(text="There is a cat.")}} |
| Il y a des nuages. | {{spoiler(text="There are clouds.")}} |
| Il y a une fête. | {{spoiler(text="There is a party.")}} |
| Il y a des pommes. | {{spoiler(text="There are apples.")}} |
| Il y a trois semaines, je suis allé à Paris. | {{spoiler(text="Three weeks ago, I went to Paris.")}} |

# Vocabulary

| French | English |
|---|---|
| école | {{spoiler(text="school")}} |
| université | {{spoiler(text="university")}} |
| parc | {{spoiler(text="park")}} |
| maison | {{spoiler(text="house")}} |
| piscine | {{spoiler(text="swimming pool")}} |
| homme | {{spoiler(text="man")}} |
| vélo | {{spoiler(text="bicycle")}} |
| sac | {{spoiler(text="bag")}} |
| stylo | {{spoiler(text="pen")}} |
| billet | {{spoiler(text="ticket")}} |
| crayon | {{spoiler(text="pencil")}} |
| avobase | {{spoiler(text="avocado")}} |
| point | {{spoiler(text="point")}} |
| montre | {{spoiler(text="watch")}} |
| voiture | {{spoiler(text="car")}} |
| bouteille | {{spoiler(text="bottle")}} |
| table | {{spoiler(text="table")}} |
| tasse | {{spoiler(text="cup")}} |
| sujet | {{spoiler(text="subject")}} |
| pronom | {{spoiler(text="pronoun")}} |
| fille | {{spoiler(text="girl")}} |
| rue | {{spoiler(text="street")}} |

## Nationalities

| Country | Definite Article | Masculine |
|---|---|---|
| **la** France | Français | Française |
| **l'** Allemagne | Allemand | Allemande |
| **l'** Angleterre | Anglais | Anglaise |
| **l'** Italie | Italien | Italienne |
| **l'** Espagne | Espagnol | Espagnole |
| **le** Canada | Canadien | Canadienne |
| **les** États-Unis | Américain | Américaine |
| **le** Maroc | Marocain | Marocaine |
| **l'** Iran | Iranien | Iranienne |
| **le** Japon | Japonais | Japonaise |
| **la** Suisse | Suisse | Suisse |
| **la** Belgique | Belge | Belge |
| **la** Suède | Suédois | Suédoise |
| **la** Russie | Russe | Russe |
| **la** Chine | Chinois | Chinoise |

## Professions

{{admonition(type='tip'
text='The following professions can be used to answer the question:

"**Quelle** est votre profession?"

Be it by themselves or in a "Je suis \*profession\*" sentence')}}

| Profession | Masculine | Feminine |
|---|---|---|
| Doctor | Médecin | Médecin |
| Lawyer | Avocat | Avocate |
| Teacher | Professeur | Professeure |
| Engineer | Ingénieur | Ingénieure |
| Nurse | Infirmier | Infirmière |
| Police officer | Policier | Policière |
| Firefighter | Pompier | Pompière |
| Accountant | Comptable | Comptable |
| Secretary | Secrétaire | Secrétaire |
| Chef | Chef | Cheffe |
| Waiter | Serveur | Serveuse |
| Artist | Artiste | Artiste |
| Writer | Écrivain | Écrivaine |
| Musician | Musicien | Musicienne |
| Actor | Acteur | Actrice |
| Dancer | Danseur | Danseuse |
| Pilot | Pilote | Pilote |
| Programmer | Programmeur | Programmeuse |
| Designer | Designer | Designere |
| Architect | Architecte | Architecte |
| Singer | Chanteur | Chanteuse |
| Director | Directeur | Directrice |
